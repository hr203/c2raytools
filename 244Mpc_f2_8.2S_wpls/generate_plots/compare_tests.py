...
def compare_temp_crosssection():

    for i in range(len(redshifts)):
#        xfrac_filename = '/lustre/scratch/astro/hr203/RESULTS/tests/20_box/Temper3D_'+str('%.3f' % redshifts[i]) + '.bin'
#        dbt_wpls = c2t.TemperFile(xfrac_filename).temper
#        data1=dbt_wpls[9,9,:]

        xfrac_filename2 = '/lustre/scratch/astro/hr203/RESULTS/tests/200_box/Temper3D_'+str('%.3f' % redshifts[i]) + '.bin'
        dbt_wpls2 = c2t.TemperFile(xfrac_filename2).temper
        data2=dbt_wpls2[99,99,:]

        xfrac_filename3 = '/lustre/scratch/astro/hr203/RESULTS/tests/10_box/Temper3D_'+str('%.3f' % redshifts[i]) + '.bin'
        dbt_wpls3 = c2t.TemperFile(xfrac_filename3).temper
        data3=dbt_wpls3[4,4,:]

        data4=np.load("tests/blurred/temp_"+str('%.3f' % redshifts[i])+".bin")
        data4=data4[4,4,:]

        print "Redshift: "+str(redshifts[i])
        #print data3

#        data4=np.zeros(len(data3))
       # for j in range(len(data2)):
       #     if j<20:
       #        data4[0] = data4[0]+data2[j]
       #     elif j<40:
       #        data4[1] = data4[1]+data2[j]
       #     elif j<60:
       #        data4[2] = data4[2]+data2[j]
       #     elif j<80:
       #        data4[3] = data4[3]+data2[j]
       #     elif j<100:
       #        data4[4] = data4[4]+data2[j]
       #     elif j<120:
       #        data4[5] = data4[5]+data2[j]
       #     elif j<140:
       #        data4[6] = data4[6]+data2[j]
       #     elif j<160:
       #        data4[7] = data4[7]+data2[j]
       #     elif j<180:
       #        data4[8] = data4[8]+data2[j]
       #     elif j<200:
       #        data4[9] = data4[9]+data2[j]
       #  data4=data4/20

    #        n=len(data2)/len(data3)
    #        data4[j/n] = data4[j/n] + data2[j]
        #data4 = data4/n

 #       distance=np.arange(2.5,7.5,5./float(len(data1)))
        distance2=np.arange(-0.95,9.05,10./float(len(data2)))
        distance3=np.arange(0,10,10./float(len(data3)))
        print len(distance2), len(data2)
...
#        print distance2
        ...
        #fig=plt.figure()
  #      plt.plot(distance,data1,color='blue',label='20')
        plt.plot(distance2,data2,color='green',label='200')
        plt.plot(distance3,data3,color='red',label='10')
        plt.plot(distance3,data4,color='blue',label='200 blurred')
        
        #for r in range(len(distance)):     
        #    plt.plot((distance[r],distance[r]),(0,20000),color='blue',alpha=0.5)
#        for i in range(len(distance2)):
#            plt.plot(temp,line,color='blue')
        for r in range(len(distance3)):
            plt.plot((distance3[r],distance3[r]),(0,20000),color='red',alpha=0.5)
        ...
        plt.xlim(0,10)
        #plt.ylim(0,1000)
        plt.savefig("tests/crosssection_"+str(redshifts[i])+".png")
        plt.close()
        #print means[i,1]
    #print redshifts_wstars[i], redshifts_wpls[i]

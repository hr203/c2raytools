...
def plot(dataslice,it,title,name,cmap='hot'):
    plt.figure()
    ...
    plt.savefig("plots/"+name+"_"+str(it)+"_"+str(redshifts[it])+".png")
    plt.close()
...
def plottemp():
    for i in range(len(redshifts)-1,-1,-1):
        ...
        plot(temperature[mesh/2,:,:],i,"Temperature","temp")

def plotdbt():
    dbt = map("dbt_"+str('%.3f' % redshifts[len(redshifts)-8]))
    maxi = findmax(dbt)
    dbt = map("dbt_"+str('%.3f' % redshifts[0]))
    mini = np.min(dbt)
    for i in range(len(redshifts)):
        dbt = map("dbt_"+str('%.3f' % redshifts[i]))
